...
//Basic Driving Function(coded by HyunSeok)
static void start_setting(int velocity);
static void Stop_wait_start_signal1(IplImage* imgYCrCb, IplImage* img_gray);
static void Stop_wait_start_signal2();
static void LineTracing(IplImage* imgHSV, IplImage* imgLine);
static void StopSignChecking(IplImage* imgHSV, IplImage* img_gray);
...
static void Stop_wait_start_signal1(IplImage* imgYCrCb, IplImage* img_gray){
	cvInRangeS(imgYCrCb, cvScalar(0, 133, 77, 0), cvScalar(255, 173, 127, 0), img_gray);
	unsigned j, k, num, count;
	count = 0;
	for (j = 0; j < RESIZE_HEIGHT; j++){
		for (k = 0; k < RESIZE_WIDTH; k++){
			if (img_gray->imageData[k + RESIZE_WIDTH * j] != 0){
				count++;
			}
		}
	}
...
static void LineTracing(IplImage* imgHSV, IplImage* imgLine){
	...
	for (i = 180; i < 240; i++){
		...
		for (j = 0; j < 320; j++){
			int now = i * 320 + j;
			imgLine->imageData[(i - 180) * 320 + j] = 255;
			if (imgHSV->imageData[now * 3] > 25 && imgHSV->imageData[now * 3] < 40){
				if (j < 180){
					left_center_LINE += j;
					left_count++;
				}
				...
				imgLine->imageData[(i - 180) * 320 + j] = 0;
			}
		}
		if (left_count < 10 && right_count < 10){
			...
			lineData[i - 180] = (int)(left_center_LINE / left_count + right_center_LINE / right_count) / 2;
		}
		imgLine->imageData[(i - 180) * 320 + lineData[i - 180]] = 0;
		datasum += lineData[i - 180];
	}
	datasum /= 60;
	if(datasum > 219) SteeringServoControl_Write(1200);
	else if(datasum < 100)SteeringServoControl_Write(1800);
	else SteeringServoControl_Write(1500);
}

static void StopSignChecking(IplImage* imgHSV, IplImage* img_gray){
	cvInRangeS(imgHSV, cvScalar(0, 0, 0, 0), cvScalar(10, 255, 255, 0), img_gray);
}
...
void *ControlThread(void *unused){
	...
	IplImage* imgYCrCb = cvCreateImage(cvSize(RESIZE_WIDTH, RESIZE_HEIGHT), IPL_DEPTH_8U, 3);
	IplImage* imgHSV = cvCreateImage(cvGetSize(imgOrigin), IPL_DEPTH_8U, 3);
	IplImage* imgLine = cvCreateImage(cvSize(320,60), IPL_DEPTH_8U, 1);
	IplImage* img_gray = cvCreateImage(cvGetSize(imgOrigin), IPL_DEPTH_8U, 1);
	...
	while (1)
	{
		...
		if (drive_status == DRIVE){
			...
			LineTracing(imgHSV, imgLine);
			StopSignChecking(imgHSV, img_gray);
			// sprintf(fileName, "captureImage/imgLine%d.png", i);
			// cvSaveImage(fileName, imgLine, 0);
		}
			...
			Stop_wait_start_signal1(imgYCrCb, img_gray);
		}
		// ---------------------------------------------------------------------
		//GetTime(&pt2);
