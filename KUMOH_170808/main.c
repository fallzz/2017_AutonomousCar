...
#define CAMERA_ANGLE 1650
#define DRIVE_VELOCITY 60
...
static void LineTracing(IplImage* imgHSV, IplImage* imgLine, IplImage* imgBird){
	...
	if (left_detection >= right_detection + 5){
		right_count = 0;
		if(++left_count > 20) SteeringServoControl_Write(1000);
		else if(left_count > 5) SteeringServoControl_Write(1300);
		else SteeringServoControl_Write(1400);
	}
	else if (right_detection >= left_detection + 5){
		left_count = 0;
		if(++right_count > 20) SteeringServoControl_Write(2000);
		else if(right_count > 5)SteeringServoControl_Write(1700);
		else SteeringServoControl_Write(1600);
	}
	else{
		left_count = 0;
		right_count = 0;
