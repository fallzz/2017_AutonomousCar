...
static void Parking_Detect(){
	...
	if(SIX_SENSOR_VALUE > 1000 && SIX_SENSOR_STATUS == 0){
		SIX_SENSOR_STATUS = 1;
		stacked_frame = 0;
	}
	else if(SIX_SENSOR_VALUE < 1000 && SIX_SENSOR_STATUS == 1){
		SIX_SENSOR_STATUS = 2;
	}
	else if(SIX_SENSOR_VALUE <= 1000 && SIX_SENSOR_STATUS == 2){
		stacked_frame++;
	}
	else if(SIX_SENSOR_VALUE > 1000 && SIX_SENSOR_STATUS == 2){
		if(stacked_frame >= 40) SIX_SENSOR_STATUS = 0;
		else if(stacked_frame < 15) drive_status = LEFT_VERTICAL_PARKING;
		else if(stacked_frame < 40) drive_status = LEFT_HORIZONTAL_PARKING;
		printf("frame : %d\n",stacked_frame);
		PARKING_STATE = 0;
		stacked_frame = 0;
	}
